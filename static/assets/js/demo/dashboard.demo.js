...
function seongnamMap(worldMapData, seongnamNetwork) {
	...
	var svg = d3.select("#seongnam-map").append("svg").attr("width", width).attr("height", height);
	var map = svg.append("g").attr("id", "map"), places = svg.append("g").attr("id", "places");
	var projection = d3.geo.mercator().center([127.1094211519, 37.409]).scale(90000).translate([width / 2, height / 2]);
